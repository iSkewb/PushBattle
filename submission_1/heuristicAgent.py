...
class ImprovedAgent:
    ...
    def evaluate_board(self, game):
        ...
        score += p1_pieces - p2_pieces

        # Potential winning moves
        score += self.evaluate_potential_wins(game, PLAYER1) - self.evaluate_potential_wins(game, PLAYER2)

        # Blocking opponent's winning moves
        score += self.evaluate_blocking_moves(game, PLAYER1) - self.evaluate_blocking_moves(game, PLAYER2)

        # Setting up two in a row
        score += self.evaluate_two_in_a_row(game, PLAYER1) - self.evaluate_two_in_a_row(game, PLAYER2)

        # Blocking opponent's two in a row
        score += self.evaluate_blocking_two_in_a_row(game, PLAYER1) - self.evaluate_blocking_two_in_a_row(game, PLAYER2)

        # Center control
        score += self.evaluate_center_control(game, PLAYER1) - self.evaluate_center_control(game, PLAYER2)
    ...
    def evaluate_center_control(self, game, player):
        ...
        for r, c in center_positions:
            if game.board[r][c] == player:
                center_control += 1
        return center_control * 2  # Give higher weight to center control
